...
>({
    ...
    // ── Content-mode: portal JSX (computed before return, used inside single return) ──
    const contentAddTitle = add?.contentTitle ?? `Tambah ${title}`;
    const contentAddDesc = add?.contentDescription ?? `Isi form berikut untuk menambahkan data baru.`;

    const contentEditTitle = (mutationMode === 'content' && view === 'edit' && edit && selectedItem)
        ? (resolve(edit.contentTitle, selectedItem) ?? `Edit ${title}`)
        : '';
    const contentEditDesc = (mutationMode === 'content' && view === 'edit' && edit && selectedItem)
        ? resolve(edit.contentDescription, selectedItem)
        : undefined;
    const handleContentBack = () => { setView('list'); setSelectedItem(null); };

    const isContentAdd = mutationMode === 'content' && view === 'add' && !!add;
    const isContentEdit = mutationMode === 'content' && view === 'edit' && !!edit && !!selectedItem;

    // ── List view (default) ───────────────────────────────────────────────────
    return (
        <div ref={containerRef} className="p-4 h-full flex flex-col gap-4">
                ...
                <div className="p-4 h-full flex flex-col sticky top-0 z-10">
                    <div className="sticky top-0 z-[999] flex items-center justify-between bg-white bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 pb-4 pt-4 mb-6 -mt-4 -mx-8 px-4 border-b">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-xl font-bold tracking-tight">{contentAddTitle}</h1>
                                <p className="text-muted-foreground text-sm">{contentAddDesc}</p>
                            </div>
                        </div>
            ...
            {/* Content-mode: Edit portal — inject into this instance's own container */}
            {isContentEdit && selectedItem && containerRef.current && createPortal(
                <div className="p-4 h-full flex flex-col sticky top-0 z-10">
                    <div className="sticky top-0 z-10 flex items-center justify-between bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 pb-4 pt-4 mb-6 -mt-4 -mx-8 px-4 border-b">
                        <div className="flex items-center gap-4">
                            <Button variant="ghost" size="icon" onClick={handleContentBack} className="h-8 w-8">
                                <ArrowLeft className="h-4 w-4" />
                            </Button>
                            <div>
                                <h1 className="text-xl font-bold tracking-tight">{contentEditTitle}</h1>
                                {contentEditDesc && <p className="text-muted-foreground text-sm">{contentEditDesc}</p>}
                            </div>
                        </div>
                        <div ref={setHeaderActionsNode} className="flex items-center gap-2">
                            {(!mutationForm || !edit?.onConfirm) && (
                                <Button variant="outline" onClick={handleContentBack}>Batal</Button>
                            )}
                        </div>
                    </div>
                    <div className="flex-1 pb-6">
                        {edit?.component
                            ? edit.component(selectedItem, handleContentBack)
                            : renderManagedEditForm(selectedItem, handleContentBack)
                        }
                    </div>
                </div>,
                containerRef.current
            ...
            {/* Page header */}
            <div className={cn("flex flex-col gap-4", (isContentAdd || isContentEdit) && "hidden")}>
                ...
                {add && (
                    mutationMode === 'modal' ? (
                        <Modal
                            ...
                            trigger={
                                <Button variant={'secondary'} className="h-10 px-4 font-semibold">
                        ...
                        <Button variant={'secondary'} className="h-10 px-4 font-semibold" onClick={() => setView('add')}>
                            <Plus className="mr-2 h-4 w-4" />
                            {add.label ?? `Tambah ${title}`}
                        </Button>
                    )
            ...
            </div>{/* end list-view hidden wrapper */}
