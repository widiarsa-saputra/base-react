    ...
    return (
        <div className="flex flex-col h-full pt-4 px-4 pb-0 children-container relative">
            <Tabs defaultValue={props.tabObjects[0].trigger} className="w-full h-full flex flex-col">
                <TabsList className="w-full bg-primary/10 relative">
                    {
                        props.tabObjects.map(({ trigger }, idx) => (
                            <TabsTrigger key={idx} value={trigger} className="font-semibold data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">{trigger}</TabsTrigger>
                        ))
                    }
                </TabsList>
