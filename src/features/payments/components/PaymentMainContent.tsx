...
const PaymentMainContent: React.FC = () => {
    ...
    return (
        <DataPageTemplate<PaymentEntity, PaymentCreatePayload>
            ...
            submitActions={{
                add: { label: 'Tambah payment', modalTitle: 'Tambah payment', onConfirm: async (data) => { await addMutation.mutateAsync(data); } },
                edit: { modalTitle: () => 'Edit payment', onConfirm: async (item, data) => { await editMutation.mutateAsync({ id: item.id ?? '', data }); } },
