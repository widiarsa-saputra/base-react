...
interface BaseUpdateSingletonProps<T extends object, R> {
...
export const useBaseUpdateSingleton = <T extends object, R, Q extends object>({
    endpoint,
    schema,
    ...
    return useMutation<R, Error, Form, { previousData: unknown }>({
        mutationFn: async (params: Form): Promise<R> => {
            const url = typeof endpoint === "function" 
                ? endpoint(params) 
                : params.id ? `${endpoint}/${params.id}` : endpoint;
        ...
        onMutate: async (form) => {
            ...
            queryClient.setQueriesData({ queryKey: [queryKey] }, (oldPost: unknown) => {
                ...
                return {
                    ...
                    data: {
                        ...old.data,
                        ...(form.data as unknown as Partial<Q>)
                    }
                }
            });
            if (query?.onMutate) {
        ...
        onSuccess: async (data, variables, context) => {
            try {
                ...
                if (!validationResult.success) {
                    ...
                    throw new Error(`Invalid ${typeof endpoint === 'string' ? endpoint : 'dynamic'} data format`);
                }
                ...
                console.error(`Failed to validate update response`, error);
                throw error;
            }
        },
